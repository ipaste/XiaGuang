...
+ (void)saveData:(FMDatabase *)database tableName:(NSString *)tableName fields:(NSArray *)fields datas:(NSArray *)datas{
    [datas enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        NSString *tmpSql = [NSString stringWithFormat:@"SELECT * FROM %@ WHERE %@ = %@",tableName,identify,contents[[fields indexOfObject:identify]]];
        FMResultSet *result = [database executeQuery:tmpSql];
        if ([result next]) {
            //更新数据
            isUpdata = true;
            sql = [NSMutableString stringWithFormat:@"UPDATA %@ SET",tableName];
        }else{
            //插入数据
        ...
        if (isUpdata){
            [sql appendFormat:@"WHERE %@ = %@",identify,contents[[fields indexOfObject:identify]]];
        }else{
            [sql appendFormat:@")"];
        }
