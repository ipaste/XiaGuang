...
@interface YTMallInfoViewController (){
    ...
    YTadScrollAndPageView *_activityView;
...
- (void)viewDidLoad{
    ...
    _activityView = [[YTadScrollAndPageView alloc]init];
...
- (void)viewWillLayoutSubviews{
    ...
    frame.origin.x = 0;
    frame.origin.y =  _activitSize.width == 0 ?  CGRectGetMaxY(_scrollButton.frame) : CGRectGetMaxY(_scrollButton.frame) + 10;
    frame.size.width = _activitSize.width;
    frame.size.height = _activitSize.height;
    _activityView.frame = frame;

    frame = _saleView.frame;
    frame.origin.x = 0;
    frame.origin.y = CGRectGetMaxY(_activityView.frame) + 10;
...
- (void)reloadActivity{
    [self orderGetImageWithCallBack:^(NSArray *results) {
        [_activityView setImages:results];
        [self.view setNeedsLayout];
    }];
}
...
- (void)orderGetImageWithCallBack:(void (^)(NSArray *results))callBack{
    ...
    [activity getActivityImgWithCallBack:^(UIImage *result, NSError *error) {
        if (!error) {
            NSMutableArray *old =  images == nil ? [NSMutableArray array]:[NSMutableArray arrayWithArray:images];
            CGFloat scale = result.size.width / CGRectGetWidth(self.view.frame);
            CGFloat pw = result.size.width / scale;
            CGFloat ph = result.size.height / scale;
            _activitSize = CGSizeMake(pw, ph);
            [old addObject:result];
            images = old.copy;
        }
        count++;
...
- (void)selectedKey:(NSString *)key{
    YTResultsViewController *resultsVC = [[YTResultsViewController alloc]initWithMall:_mall Key:key];
    [self.navigationController pushViewController:resultsVC animated:YES];
}
